...
void Interpreter::checkNumberOperand(const Token& oper, const std::any& operand){
  ...
  throw RuntimeError{oper, "O operando deve ser um numero."};
}
...
int64_t Interpreter::doubleToInt(const Token& oper, const std::any& value) {
  ...
  if (modf(std::any_cast<double>(value), &integerPart) != 0.0) {
    throw RuntimeError{oper, "O operando deve ser um inteiro."};
  }
  if (integerPart < static_cast<double>(std::numeric_limits<int64_t>::min()) ||
    ...
    throw RuntimeError{oper, "Valor fora do intervalo int64_t"};
  }
...
std::string Interpreter::stringify(const std::any& object){
  if(object.type() == typeid(nullptr)) return "nulo";
  ...
  if(object.type() == typeid(bool)){
    if(std::any_cast<bool>(object)){
      std::string result{"verdadeiro"};
      return result;
    }else{
      std::string result{"falso"};
      return result;
    }
  }
  ...
  return "ache_texto: não foi possível reconhecer o tipo";
}
...
std::any Interpreter::visitVariableExpr(std::shared_ptr<Variable> expr){
  ...
  if(value.type() == typeid(nullptr)){
    throw RuntimeError(expr->name, "Variavel nao inicializada");
  }
...
std::any Interpreter::visitCallExpr(std::shared_ptr<Call> expr){
  ...
  throw RuntimeError{expr->paren, "So e possível chamar funcoes e classes."};
}
...
std::any Interpreter::visitGetExpr(std::shared_ptr<Get> expr){
  ...
  throw RuntimeError(expr->name, "Somente instancias possuem propriedades.");
}

std::any Interpreter::visitSetExpr(std::shared_ptr<Set> expr){
  std::any object = evaluate(expr->object);
  if(object.type() != typeid(std::shared_ptr<Instance>)){
    throw RuntimeError(expr->name, "Somente instâncias possuem propriedades.");
  }
  std::any value = evaluate(expr->value);
  auto instance = std::any_cast<std::shared_ptr<Instance>>(object);
    ...
    if(index.type() == typeid(double)){
      ...
      if(expr->value != nullptr){
        ...
        if(list->setAtIndex(static_cast<int>(castedIndex), value)) {
          ...
          throw RuntimeError{expr->paren, "Indice fora do intervalo."};
        }
      }else{
      ...
      throw RuntimeError{expr->paren, "O Indice deve ser do tipo inteiro."};
    }
  }else{
    throw RuntimeError{expr->paren, "Somente vetores podem ser chamados."};
  }
  return {};
}
