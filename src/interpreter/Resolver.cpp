...
void Resolver::declare(Token& name){
  ...
  if(currentScope.find(name.lexeme) != currentScope.end()){
    Debug::error(name, "Nao sao permitidas multiplas variaveis com o mesmo nome.");
  }
  currentBlock[name] = 0;
...
std::any Resolver::visitVariableExpr(std::shared_ptr<Variable> expr){
  if(!scopes.empty()){
    ...
    if(elem != currentScope.end() && elem->second == false){
      Debug::error(expr->name, "Nao e possivel ler a variável local neste inicializador proprio.");
    }
  }
...
std::any Resolver::visitReturnStmt(std::shared_ptr<Statement::Return> stmt){
  if(currentFunction == FType::NONE){
    Debug::error(stmt->keyword, "Nao e possivel retornar do codigo de nivel superior.");
  }
